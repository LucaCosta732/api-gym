...
import it.lucacosta.gym.dto.EsercizioDto; // Assumendo che EsercizioDto esista
...
public interface EsercizioController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Esercizio trovato"),
            @ApiResponse(responseCode = "404", description = "Esercizio non trovato")
    })
    @GetMapping("/{id}")
    public ResponseEntity<EsercizioDto> getEsercizioById(
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Lista degli esercizi restituita con successo")
    })
    @GetMapping("/")
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Esercizio creato con successo"),
            @ApiResponse(responseCode = "400", description = "Richiesta non valida")
    })
    @PostMapping("/add")
    public ResponseEntity<EsercizioDto> addEsercizio(@RequestBody EsercizioDto esercizioDto);

    @Operation(summary = "Aggiungi più esercizi", description = "Crea e aggiunge una lista di nuovi esercizi al sistema.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Esercizi creati con successo"),
            @ApiResponse(responseCode = "400", description = "Richiesta non valida")
    })
    @PostMapping("/addAll")
    public ResponseEntity<List<EsercizioDto>> addEsercizi(@RequestBody List<EsercizioDto> eserciziDto);

    @Operation(summary = "Aggiorna un esercizio esistente", description = "Aggiorna i dettagli di un esercizio esistente.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Esercizio aggiornato con successo"),
            @ApiResponse(responseCode = "404", description = "Esercizio non trovato"),
            @ApiResponse(responseCode = "400", description = "Richiesta non valida")
    })
    @PutMapping("/update")
    public ResponseEntity<EsercizioDto> updateEsercizio(@RequestBody EsercizioDto esercizioDto);

    @Operation(summary = "Elimina un esercizio", description = "Elimina un esercizio dal sistema tramite il suo ID.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Esercizio eliminato con successo"),
            @ApiResponse(responseCode = "404", description = "Esercizio non trovato")
    })
    @DeleteMapping("/delete/{id}")
    public ResponseEntity<Boolean> deleteEsercizio(@PathVariable Long id);
}